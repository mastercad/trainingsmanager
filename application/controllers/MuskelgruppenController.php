...
class MuskelgruppenController extends Zend_Controller_Action
{
    ...
    public function indexAction()
    {
        ...
        $obj_db_muskelgruppe_muskeln = new Application_Model_DbTable_MuskelgruppeMuskeln();
        
        $a_muskelgruppen = $obj_db_muskelgruppen->getMuskelgruppen();
        
        foreach($a_muskelgruppen as &$a_muskelgruppe)
        {
            $a_muskelgruppe_muskeln = $obj_db_muskelgruppe_muskeln->getMuskelnFuerMuskelgruppe($a_muskelgruppe['muskelgruppe_id']);
    ...
    public function loescheMuskelgruppeAction()
    {
           ...
           $a_params['id'] > 0)
        {
            ...
            if($obj_db_muskelgruppen->loescheMuskelgruppe($i_muskelgruppe_id))
            {
                $obj_db_muskelgruppen_muskeln->loescheAlleMuskelgruppeMuskelnFuerMuskelgruppe($i_muskelgruppe_id);
    ...
    public function getMuskelnFuerEditAction()
    {
        ...
        if(isset($a_params['id']))
        {
            ...
            $a_muskelgruppe_muskeln = $obj_db_muskelgruppe_muskeln->getMuskelnFuerMuskelgruppe($i_muskelgruppe_id);
    ...
    */
    public function getMuskelgruppenVorschlaegeAction()
    {
        ...
        if(isset($a_params['suche']))
        {
            ...
            $a_muskelgruppen_vorschlaege = $obj_db_muskelgruppen->getMuskelgruppenByName($str_suche);
    ...
    public function speichernAction()
    {
        ...
        if(isset($a_params['edited_elements']))
        {
            ...
            if(isset($a_params['edited_elements']['muskelgruppe_muskeln']))
            {
                ...
                $obj_db_muskelgruppe_muskeln = new Application_Model_DbTable_MuskelgruppeMuskeln();
                $a_muskelgruppe_muskeln_aktuell_roh = $obj_db_muskelgruppe_muskeln->getMuskelnFuerMuskelgruppe($i_muskelgruppe_id);
...
//                            array_push($a_muskelgruppe_muskeln_loeschen, $a_muskelgruppe_muskeln_aktuell[$a_muskel['id']]);
                            array_push($a_muskelgruppe_muskeln_loeschen, $a_muskelgruppe_muskeln_aktuell[$a_muskel['id']]['muskelgruppe_muskel_id']);
...
//                            $a_muskel['beanspruchung'] > 0)
            ...
            if(0 == strlen(trim($muskelgruppe_name)) &&
               !$i_muskelgruppe_id)
            {
                array_push($a_messages, array('type' => 'fehler', 'message' => 'Diese Muskelgruppe benötigt einen Namen'));
                $b_fehler = true;
            }
            ...
            {
                array_push($a_messages, array('type' => 'fehler', 'message' => 'Diese Muskelgruppe benötigt mindestens einen beanspruchten Muskel'));
                $b_fehler = true;
            }
            ...
            {
                $a_muskelgruppe_aktuell = $obj_db_muskelgruppen->getMuskelgruppenByName($muskelgruppe_name);
                if(is_array($a_muskelgruppe_aktuell) &&
                   count($a_muskelgruppe_aktuell) > 0)
                {
                    array_push($a_messages, array('type' => 'fehler', 'message' => 'Muskelgruppe "' . $muskelgruppe_name . '" existiert bereits!', 'result' => false));
                    $b_fehler = true;
                }
            }
            ...
            if(!$b_fehler) {
                ...
                if(is_numeric($i_muskelgruppe_id) &&
                   ...
                   count($a_data) > 0) {
                    ...
                    $a_muskelgruppe_aktuell = $obj_db_muskelgruppen->getMuskelgruppe($i_muskelgruppe_id);
                    ...
                    $obj_db_muskelgruppen->updateMuskelgruppe($a_data, $i_muskelgruppe_id);
                    array_push($a_messages, array('type' => 'meldung', 'message' => 'Diese Muskelgruppe wurde erfolgreich bearbeitet!', 'result' => true, 'id' => $i_muskelgruppe_id));
                }
                // neu anlegen
                    ...
                    $i_muskelgruppe_id = $obj_db_muskelgruppen->setMuskelgruppe($a_data);
                    array_push($a_messages, array('type' => 'meldung', 'message' => 'Diese Muskelgruppe wurde erfolgreich angelegt!', 'result' => true, 'id' => $i_muskelgruppe_id));
                }
                ...
                {
                    array_push($a_messages, array('type' => 'meldung', 'message' => 'Diese Muskelgruppe wurde nicht geändert!', 'result' => true, 'id' => $i_muskelgruppe_id));
                }
                ...
                if(count($a_muskelgruppe_muskeln_hinzufuegen) > 0 ||
                        ...
                        count($a_muskelgruppe_muskeln_loeschen) > 0)
                {
                    array_push($a_messages, array('type' => 'meldung', 'message' => 'Die Muskeln der Muskelgruppen wurden erfolgreich geändert!', 'result' => true, 'id' => $i_muskelgruppe_id));
                }
...
//                    $str_dest_path = getcwd() . '/images/content/dynamisch/muskelgruppen/' . $i_muskelgruppe_id . '/';
                    ...
                    */
                    ...
                    foreach($a_muskelgruppe_muskeln_hinzufuegen as $a_muskelgruppe_muskel)
                    {
                        ...
                        $obj_db_muskelgruppe_muskeln->setMuskelgruppeMuskel($a_data);
                    }
                    ...
                    foreach($a_muskelgruppe_muskeln_updaten as $a_muskelgruppe_muskel)
                    {
                        ...
                        $obj_db_muskelgruppe_muskeln->updateMuskelgruppeMuskel($a_data, $a_muskelgruppe_muskel['muskelgruppe_muskel_id']);
                    }
                    
                    foreach($a_muskelgruppe_muskeln_loeschen as $i_muskelgruppe_muskel_id)
                    {
                        $obj_db_muskelgruppe_muskeln->loescheMuskelAusMuskelgruppe($i_muskelgruppe_muskel_id);
                    }
                }
            }
            else
            {
                array_push($a_messages, array('type' => 'fehler', 'message' => 'Es gabe einen Fehler bei Muskelgruppe speichern!', 'result' => false));
            }
        }
        else
        {
            array_push($a_messages, array('type' => 'fehler', 'message' => 'Falscher Aufruf von Muskelgruppe speichern!', 'result' => false));
        }
        $this->view->assign('json_string', json_encode($a_messages));
    }
}
