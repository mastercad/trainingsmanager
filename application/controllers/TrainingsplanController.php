...
class TrainingsplanController extends Zend_Controller_Action {
    ...
    public function indexAction()
    {
        ...
        $oTrainingsplaeneRowSet = $oTrainingsplaeneStorage->findAllActiveTrainingPlans();
    ...
    {
        $oTrainingsplaeneStorage = new Application_Model_DbTable_TrainingPlans();
        $oTrainingsplaeneRowSet = $oTrainingsplaeneStorage->findAllInactiveTrainingPlans();

        if (FALSE !== $oTrainingsplaeneRowSet) {
            $sContent = '';
            foreach ($oTrainingsplaeneRowSet as $oTrainingsplanRow) {
                $sTrainingsplanName = $oTrainingsplanRow->user_vorname . ' - ' .
                    date('d.m.Y', strtotime($oTrainingsplanRow->trainingsplan_eintrag_datum));
    ...
    public function showAction() {
        ...
        if (array_key_exists('id', $aParams)
            ...
            $oTrainingsplaeneStorage = new Application_Model_DbTable_TrainingPlans();
            $oTrainingstagebuecherStorage = new Application_Model_DbTable_TrainingDiaries();
            $oTrainingsplanUebungenStorage = new Application_Model_DbTable_TrainingPlanExercises();
            ...
            if (2 == $oTrainingsplanRow->trainingsplan_layout_fk) {
                $oTrainingsplanChildren =
                    $oTrainingsplaeneStorage->findChildTrainingPlans($iTrainingsplanId);

                foreach ($oTrainingsplanChildren as $oTrainingsplanChild) {
                    $sUebungen = '';
                    $oTrainingsplanUebungen = $oTrainingsplanUebungenStorage->findExercisesByTrainingPlanId(
                        $oTrainingsplanChild->trainingsplan_id);

                    foreach ($oTrainingsplanUebungen as $oTrainingsplanUebung) {
                        $oTrainingstagebuchUebung =
                            $oTrainingstagebuecherStorage->findActualTrainingsDiaryByTrainingPlanExerciseId(
                                $oTrainingsplanUebung->trainingsplan_uebung_fk);
                        ...
                        $oBeanspruchteMuskelnFuerUebung = $oUebungMuskelnStorage->findMusclesForExercise($oTrainingsplanUebung->trainingsplan_uebung_fk);
                ...
                $sUebungen = '';
                $oTrainingsplanUebungen = $oTrainingsplanUebungenStorage->findExercisesByTrainingPlanId($iTrainingsplanId);
                foreach ($oTrainingsplanUebungen as $oTrainingsplanUebung) {
                    $oTrainingstagebuchUebung = $oTrainingstagebuecherStorage->findActualTrainingsDiaryByTrainingPlanExerciseId($oTrainingsplanUebung->trainingsplan_uebung_fk);
                    $this->considerTrainingstagebuchEntryFuerUebung($oTrainingsplanUebung, $oTrainingstagebuchUebung);
    ...
    /*
     *  setzt die aktuellen werte entweder aus dem trainingsplan,
     *  oder aus dem trainingstagebucheintrag für eine übung
     */
        ...
        if (array_key_exists('id', $aParams)
            ...
            if (2 == $oTrainingsplanRow->trainingsplan_layout_fk) {
                ...
                $oTrainingsplanUebungen =
                    $oTrainingsplanUebungenStorage->findExercisesByParentTrainingPlanId($iTrainingsplanId);
    ...
    public function speichernAction()
    {
        ...
        if (TRUE === array_key_exists('trainingsplan_uebung_id', $aParams)) {
            $oTrainingsplanUebungen = new Application_Model_DbTable_TrainingPlanExercises();
...
//                            $aData['trainingsplan_uebung_order']            = $aParams['trainingsplan_uebung_order'][$iTrainingsplanId][$iTrainingsplanUebungId];
    ...
    {
        $oTrainingsplanLayouts = new Application_Model_DbTable_TrainingPlanLayouts();
    ...
    public function getTrainingsplanFuerSplitAction()
    {
        ...
        $oUser = Zend_Auth::getInstance()->getIdentity();
        $oTrainingsplaene = new Application_Model_DbTable_TrainingPlans();

        if (TRUE === is_object($oUser)) {
            $iUserId = $oUser->user_id;
            ...
            if (is_numeric($iUserId)
                ...
                $sContent = json_encode(array(array('type' => 'fehler', 'message' => 'Konnte Trainingsplan nicht anlegen!')));
                $this->view->assign('sContent', $sContent);
            }
        } else {
            $sContent = json_encode(array(array('type' => 'fehler', 'message' => 'Für diese Aktion müssen Sie angemeldet sein!')));
            $this->view->assign('sContent', $sContent);
        }
    }
    ...
    public function getUebungAction()
    {
        ...
        if (array_key_exists('id', $aParams)) {
            ...
            $oUebungen = new Application_Model_DbTable_Exercises();
            $oUebungRow = $oUebungen->findExerciseById($iUebungId);
    ...
    public function getUebungenVorschlaegeAction()
    {
        ...
        if (array_key_exists('suche', $aParams)) {
            ...
            $oUebungenStorage = new Application_Model_DbTable_Exercises();
            $oUebungenRows = $oUebungenStorage->findExercisesByName('%' . $sSearch . '%');
            $sContent = '';
    ...
    {
        $oUebungDb = new Application_Model_DbTable_Exercises();
        return $oUebungDb->findExerciseById($iUebungId);
    }
}
