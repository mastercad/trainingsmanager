            ...
            jQuery(this).on('keyup', function(e)
            {
                ...
                post_request = jQuery.ajax({
                    ...
                    success: function(response){
                        ...
                        jQuery('body').append(jQuery('.device-proposals'));
                        jQuery('.device-proposals')
                            .html(response)
                            .css('left', offset.left + "px")
                            .css('top', offset.top + 10 + "px")
                            .find('.proposal')
                            .unbind('click')
                            .bind('click', function() {
                            // check ob diese geraetegruppe bereits gesetzt wurde
                                ...
                                jQuery('.device-proposals').fadeOut();
                                return false;
                            })
                            .fadeIn();
                    }
            ...
            jQuery(this).on('keyup', function(e)
            {
                ...
                post_request = jQuery.ajax({
                    ...
                    success: function(response){
                        var offset = jQuery(self).offset();
                        jQuery('body').append(jQuery('.device-proposals'));
                        jQuery('.device-proposals').html(response);
                        jQuery('.device-proposals').css('left', offset.left + "px");
                        jQuery('.device-proposals').css('top', offset.top + 10 + "px");

                        jQuery('.device-proposals').find('.proposal').unbind('click').bind('click', function()
                        {
                            // check ob diese geraetegruppe bereits gesetzt wurde
                        ...
                        jQuery('.device-proposals').fadeIn();
                    }
                });
            });
        });
...
//                                jQuery(self).data('muskelgruppe-id', jQuery(this).data('vorschlag-id'));
    ...
    function initMusclesEdit()
    {
        ...
        jQuery('.muscles-in-muscle-group').find('.muscle-name').each(function()
        {
            ...
            jQuery(this).on('keyup', function(e)
            {
                ...
                post_request = jQuery.ajax({
                    ...
                    success: function(response){
                        var offset = jQuery(self).offset();
                        jQuery('body').append(jQuery('.muscle-proposals'));
                        jQuery('.muscle-proposals').html(response);
                        jQuery('.muscle-proposals').css('left', offset.left + "px");
                        jQuery('.muscle-proposals').css('top', offset.top + 10 + "px");

                        jQuery('.muscle-proposals').find('.proposal').unbind('click');
                        jQuery('.muscle-proposals').find('.proposal').bind('click', function()
                        {
                            // check ob diese muskelgruppe bereits gesetzt wurde
                            var vorhandene_muskeln = jQuery('.muscles-in-muscle-group').find('.muscle');
                            for(var i = 0, len = vorhandene_muskeln.length; i < len; i++)
                            {
                                if(jQuery(vorhandene_muskeln[i]).find('.muscle-name').data('muscle-id') ==
                                    jQuery(this).data('proposal-id'))
                                {
                        ...
                        jQuery('.muscle-proposals').fadeIn();
                    }
                });
            });
        });
        ...
        jQuery('.muscle-add').unbind('click').bind('click', function()
        {
            ...
            var url = '/muscles/get-muscles-for-edit/';
            var obj_params = {'ajax': true};
