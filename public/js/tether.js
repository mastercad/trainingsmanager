...
// Remove any elements we were using for convenience from the DOM
  ...
  updateAttachClasses(elementAttach, targetAttach); {
    ...
    const all = [];
    sides.forEach(side => {
        all.push(`${ this.getClass('element-attached') }
    -
    ${ side }
`)
    all.push(`${ this.getClass('target-attached') }
    -
    ${ side }
`)
}
...
const targetSize = targetPos;
    ...
    if (typeof this.options.optimizations !== 'undefined' &&
      ...
      ['Top', 'Left', 'Bottom', 'Right'].forEach(side => {
        ...
        = parseFloat(offsetParentStyle[`border$
        {
            side
        }
        ;
  ...
  move(pos); {
    ...
    const transcribe = (_same, _pos) => {
      ...
      if (gpu !== false) {
        ...
        css[transformKey] =; `translateX(${ Math.round(xPos) }px);
          translateY(${ Math.round(yPos) }px)`
